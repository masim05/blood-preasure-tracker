...
Per constitution policy, feature implementation MUST run in a dedicated worktree
under `tmp/` after branch creation.
...
After branch creation, create/use a dedicated worktree under `tmp/`, for example:

```bash
git worktree add tmp/<branch-name> <branch-name>
```

Do not implement features from the main checkout.
