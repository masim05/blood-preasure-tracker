...
## POST /api/v1/measurements/<id>/override

Overrides one or more recognized readings for an owned measurement before or after save.
...
- At least one of `systolic`, `diastolic`, or `pulse` is required.
- Each provided field must be a positive integer.
- No additional request properties are allowed.
...
- Recognized measurements return the same shape with `"status": "recognized"` and `"savedAt": null`.
- Override responses do not include `imageUrl`.

**Failure cases**:
- `400` for empty bodies, non-positive or non-integer override values, or unexpected request properties.
- `401` for missing, expired, revoked, or invalid bearer token.
- `404` when the measurement does not exist or is not owned by the authenticated user.
- `409` when the measurement is neither recognized nor saved.
