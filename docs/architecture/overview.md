...
Operational logging is part of the web server boundary. `loadApiLoggingConfig` maps `NODE_ENV=production` to warn-or-higher Nest log levels and every other value to debug-capable development logging. `HttpRequestLoggingMiddleware` emits one JSON request/status log entry after each completed request, including method, path, status code, and duration. Unexpected HTTP failures additionally emit a sanitized error diagnostic with safe request metadata and normalized exception name, message, and stack. These log entries intentionally exclude request bodies, response bodies, authorization headers, cookies, passwords, multipart payloads, image bytes, and recognized health data; non-`Error` thrown values are never serialized.
