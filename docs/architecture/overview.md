...
Runtime configuration is read from environment variables and optional `.env` loading in `ApiConfigService`. Required runtime inputs include `DATABASE_URL`; server behavior also depends on `API_PORT`, `MEASUREMENT_IMAGE_DIR`, `ACCESS_TOKEN_TTL_SECONDS`, `RECOGNITION_WORKER_INTERVAL_SECONDS`, `RECOGNITION_WORKER_BATCH_SIZE`, `RECOGNITION_TASK_LEASE_TIMEOUT_SECONDS` (default `600`), `RECOGNITION_MAX_ATTEMPTS` (default `3`), `OPENAI_API_KEY`, provider/model settings used by the recognition adapter, and `NODE_ENV` for logging mode. The worker recovers processing attempts whose lease expires and fails the task and recognizing measurement when the configured total-attempt limit is exhausted.
